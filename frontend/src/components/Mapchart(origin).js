                      ...
                      return (
                        <Geography
                          ...
                          onClick={() => {
                            ...
                            if (!isZoom) {
                              const centroid = geoCentroid(geo); // -> 선택된 지도(geo)의 중심좌표를 구하는 함수
                              setMap(GwanakMap); // 관악구를 클릭하면 관악구 지도로 변경 된다.
                              setCenter(centroid); // 지도가 보여지는 화면의 중심좌표 ( centroid -> 중심좌표 )
                              setIsZoom(!isZoom); // 줌 상태인지 확인 (toggle 변수 )
                              setZoomLevel(MAX_ZOOM); // max_zoom = 5
                            } else {
                              setIsZoom(!isZoom);
                              setMap(SeoulMap);
