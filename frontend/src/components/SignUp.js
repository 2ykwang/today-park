  ...
  <div className="SignUpContainer">
    ...
    <form>
      ...
      <div className="signUpItem">
        ...
        <input type='password' value={password} onChange={getPassword} placeholder="영어 대/소문자, 숫자, 특수문자 포함 8글자 이상"/>
      </div>
      <p className="caution" style={{fontSize: '12px'}}>{password !== "" && !(regExpPw.test(password)) ? "형식에 맞지 않는 비밀번호 입니다." : undefined }</p>
      ...
      { pwCheck ?
      <p className="caution" style={{fontSize: '12px'}}>{pwCheck !== '' && password === pwCheck ? "비밀번호가 일치합니다" : "비밀번호가 일치하지 않습니다"}</p> 
      : undefined}
