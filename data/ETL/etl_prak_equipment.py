...
class Park_Equipment_Gwanak() :
    ...
    def split_re_for_gwanak(self, x) :
        ...
        p1 = re.compile('([ㄱ-ㅎㅏ-ㅣ-가-힣\s0-9\(\)]+)[\+]?\s?([ㄱ-ㅎㅏ-ㅣ-가-힣\s0-9\(\)]+)?\S?(\([0-9]개\))')
        ...
        for s in spl :
            ...
            if regexStr is None :
                ...
                # 1~ : 그룹 1~
                # p2 저장
                self.arr_ParkEquip.append([x['명칭'],(str(regexStr.group(1)).strip()),1])
                # + 로 두개의 운동기가 엮이는 경우가 있음 그런 경우
                if regexStr.group(2) is not None :
                    self.arr_ParkEquip.append([x['명칭'],(str(regexStr.group(2)).strip()),1])
            else :
                #p1저장
                #g1~g2 : 운동기구, g3 : 수량
                self.arr_ParkEquip.append([ x['명칭'], (str(regexStr.group(1)).strip()), int(str(regexStr.group(3)).strip()[1:-2]) ])
                if regexStr.group(2) is not None :
                    self.arr_ParkEquip.append([ x['명칭'], (str(regexStr.group(2)).strip()), int(str(regexStr.group(3)).strip()[1:-2]) ])
    ...
    def etl_data(self) :
        ...
        #columns rename
        df_park_equip.rename(columns={0:'park_name',1:'before_equipment_name',2:'quantity'}, inplace=True)

        #운동 기기명 변환
        df_park_equip['equipment_name'] = df_park_equip['before_equipment_name'].replace(fitness_equipment)
        ...
        df_park_equip['park_id'] = df_park_equip['park_name'].apply(lambda x: self.attach_park_id(x, df_serach_park))
        df_park_equip['equipment_id'] = df_park_equip['equipment_name'].apply(lambda x: self.attach_equip_id(x, df_serach_equip))

        #필요 없는 컬럼 삭제
        df_park_equip.drop(columns=['before_equipment_name','park_name','equipment_name'], inplace=True)
